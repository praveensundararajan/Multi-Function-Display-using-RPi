...
class InstrumentPanel(QWidget):
    ...
    def initializeGauges(self):
        ...
        self.oil_display = LargeNumberDisplay(vals["oil"], "°F", "Oil")

        # Initialize other displays
        self.cht_display = LabeledValueDisplay("CHT °F", "352.340")
        self.egt_display = LabeledValueDisplay("EGT °F", "1335.1274")
    ...
    def createNavigationButtons(self):
        ...
        for i in range(6):
            ...
            if i == 0:
                btn.setChecked(True)
            if i < 4:
                btn.clicked.connect(lambda checked, page=i + 1: self.switchPage(page))
            nav_layout.addWidget(btn)
            if i < 5:  # Add spacer after each button except the last one
                nav_layout.addItem(
    ...
    def initUI(self):
        ...
        instrument_layout.addWidget(self.power_display, 0, 1, 1, 1)
        instrument_layout.addWidget(self.rpm_display, 0, 2, 1, 1)
        instrument_layout.addWidget(self.man_hg_display, 0, 3, 1, 1)
        instrument_layout.addWidget(self.fflow_gph_display, 0, 4, 1, 1)
        instrument_layout.addWidget(self.oil_display, 0, 5, 1, 1)

        instrument_layout.addWidget(self.fuel_flow_1, 1, 1, 1, 2)
        instrument_layout.addWidget(self.fuel_flow_2, 2, 1, 1, 2)
        instrument_layout.addWidget(self.fuel_flow_3, 3, 1, 1, 2)
        instrument_layout.addWidget(self.fuel_flow_4, 4, 1, 1, 2)
